...
export function authFlowsTools(client: KeycloakClient): ToolDefinition[] {
  return [
    {
    ...
    },
    {
      ...
      handler: async (input) => {
        ...
        const flows = await client.get<JsonObject[]>(client.realmPath(realm, "/authentication/flows"));
        const flow = flows.find((candidate) => candidate.alias === flowAlias);
        if (!flow) throw new Error(`Authentication flow '${flowAlias}' was not found.`);
