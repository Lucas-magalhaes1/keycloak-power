...
const DEFAULT_ROLE_NAMES: ReadonlySet<string> = new Set(DEFAULT_ROLES.map((role) => role.name));

export function saasRolesTools(client: KeycloakClient): ToolDefinition[] {
  return [
    {
    ...
    },
    {
      ...
      inputSchema: objectSchema({
        ...roleTargetSchema().properties as JsonObject,
      ...
      handler: async (input) => {
        ...
        for (const desired of DEFAULT_ROLES) {
          ...
          try {
            ...
            if (!(error instanceof KeycloakApiError) || error.status !== 409) throw error;
          }
          roles = await listClientRoles(client, target.realm, clientUuid);
    ...
    },
    {
      ...
      handler: async (input) => {
        ...
        const mappings = await client.get<JsonObject[]>(mappingPath);
...
function roleTargetSchema(): JsonObject {
  return objectSchema({
...
function roleAssignmentSchema(): JsonObject {
  return objectSchema({
    realm: schema.string("Target realm name."),
...
function readTarget(input: JsonObject): { realm: string; clientId: string } {
  return { realm: expectString(input, "realm"), clientId: expectString(input, "clientId") };
}

function readAssignment(input: JsonObject, operation: string): {
  realm: string;
  userId: string;
  clientId: string;
  roleName: string;
...
async function listClientRoles(client: KeycloakClient, realm: string, clientUuid: string): Promise<JsonObject[]> {
  return client.get<JsonObject[]>(client.realmPath(realm, `/clients/${encodeURIComponent(clientUuid)}/roles`));
}

async function getClientRole(client: KeycloakClient, realm: string, clientUuid: string, roleName: string): Promise<JsonObject> {
  const role = await client.get<JsonObject>(client.realmPath(realm, `/clients/${encodeURIComponent(clientUuid)}/roles/${encodeURIComponent(roleName)}`));
  return role;
}

function rolePlan(target: { realm: string; clientId: string }, clientUuid: string, roles: JsonObject[]): JsonObject {
