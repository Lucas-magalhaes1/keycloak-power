...
export function rolesTools(client: KeycloakClient): ToolDefinition[] {
  return [
    {
      name: "list_roles",
      ...
      handler: async (input) => {
        ...
        if (!clientId) return client.get(client.realmPath(realm, "/roles"));
    ...
    },
    {
      ...
      handler: async (input) => {
        ...
        const name = expectString(input, "name");
        const clientId = optionalString(input, "clientId");
        if (!clientId) return client.get(client.realmPath(realm, `/roles/${encodeURIComponent(name)}`));
        const clientUuid = await client.resolveClientUUID(realm, clientId);
