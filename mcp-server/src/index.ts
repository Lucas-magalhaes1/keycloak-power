...
const server = new Server(
  { name: "keycloak-admin", version: "1.0.0" },
  { capabilities: { tools: {}, resources: {} }, instructions: "Keycloak Admin REST API tools. Mutations require a least-privilege service account." },
);
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  if (!tool) return toolError(`Unknown Keycloak tool '${request.params.name}'.`);
  const input = request.params.arguments ?? {};
  if (typeof input !== "object" || input === null || Array.isArray(input)) {
    return toolError("Tool arguments must be a JSON object.");
  }
  try {
    const result = await tool.handler(input as JsonObject);
...
function formatError(error: unknown): string {
  if (error instanceof KeycloakApiError) return error.message;
  return `Keycloak tool failed: ${messageFrom(error)}`;
}

function apiDocumentation(): JsonObject {
  return {
