import { KeycloakApiError, expectConfirmation, expectString, extractServerVersion, objectSchema, redactSensitive, schema, } from "../client.js";
...
export function userFederationTools(client) {
    return [
        {
            ...
            handler: async (input) => {
                ...
                const providers = await client.get(client.realmPath(realm, "/components"), { type: USER_STORAGE_PROVIDER_TYPE });
        ...
        },
        {
            ...
            handler: async (input) => {
                ...
                await requireFederationIntegration(client);
                const provider = await client.get(client.realmPath(realm, `/components/${encodeURIComponent(providerId)}`));
        ...
        },
        {
            ...
            handler: async (input) => {
                ...
                const provider = await client.get(client.realmPath(realm, `/components/${encodeURIComponent(providerId)}`));
                assertUserStorageProvider(provider);
                const mappers = await client.get(client.realmPath(realm, "/components"), { parent: providerId });
...
async function requireFederationIntegration(client) {
    ...
    if (!actual)
        throw new KeycloakApiError(412, "Keycloak server version could not be determined; user-federation REST adapters remain disabled.");
    if (!tested) {
...
function assertUserStorageProvider(provider) {
...
function redactFederation(provider) {
    return redactSensitive(provider, { redactInternalUrls: true, redactPii: true });
}
function rejectCredentialInput(input) {
    const supplied = Object.keys(input).map((key) => key.replace(/[._-]/g, "").toLowerCase()).filter((key) => forbiddenInputKeys.has(key));
    if (supplied.length > 0)
        throw new KeycloakApiError(400, "Bind passwords and credentials are never accepted by test_user_federation_connection.");
}
...
//# sourceMappingURL=user-federation.js.map